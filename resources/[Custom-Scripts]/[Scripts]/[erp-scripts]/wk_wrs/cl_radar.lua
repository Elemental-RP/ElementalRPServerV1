    ...
    if ( IsPedSittingInAnyVehicle( ped ) ) then 
        ...
        if ( GetVehicleClass( vehicle ) == 18 ) then
            ...
            if ( radarEnabled ) then 
                Notify( "~b~Radar Ativado." )
            else 
                Notify( "~b~Radar Desativado." )
            end 
            ...
            Notify( "~r~Tens de estar numa viatura da policia." )
        end 
    else 
        Notify( "~r~Tens de estar num carro." )
    end 
end )
...
function ToggleSpeedType()
    if ( radarInfo.speedType == "mph" ) then 
        ...
        Notify( "~b~Velociade setada para Km/h." )
    else 
        radarInfo.speedType = "mph"
        Notify( "~b~Velociade setada para MPH." )
    end
end 
...
Citizen.CreateThread( function()
    while true do 
        ...
        if ( IsDisabledControlPressed( 1, 36 ) and IsDisabledControlJustPressed( 1, 57 ) ) then 
            TriggerEvent( 'wk:radarRC' )
        end 

        -- LCtrl is not being pressed and M has just been pressed 
        if ( not IsDisabledControlPressed( 1, 36 ) and IsDisabledControlJustPressed( 1, 57 ) ) then 
            ResetFrontFast()
            ResetRearFast()
        end 
