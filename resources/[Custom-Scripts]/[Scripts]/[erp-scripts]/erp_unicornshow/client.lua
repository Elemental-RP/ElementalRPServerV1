	...
	ESX.UI.Menu.Open(
		...
		function(data, menu)
      	if data.current.label == 'Personne'	then
      		...
      		TriggerServerEvent('erp_unicornshow:DeleteShow',position.label)
      	else
...
function showUni(dance,dancer,position)
  ...
  TriggerServerEvent('erp_unicornshow:RegisterShow',param)
...
RegisterNetEvent('erp_unicornshow:StopDance')
AddEventHandler('erp_unicornshow:StopDance',function(ped)
  ClearPedTasksImmediately(ped)
  DeletePed(ped)
end)


AddEventHandler('erp_unicornshow:hasEnteredMarker', function(zone)

	CurrentAction     = 'strip_menu'
	CurrentActionMsg  = 'pressione ~INPUT_CONTEXT~ para escolher os shows'
	CurrentActionData = {zone = zone}


end)

AddEventHandler('erp_unicornshow:hasExitedMarker', function(zone)
...
Citizen.CreateThread(function()
	while true do
		...
		if isInMarker and not HasAlreadyEnteredMarker then
			...
			TriggerEvent('erp_unicornshow:hasEnteredMarker', currentZone)
		end
		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('erp_unicornshow:hasExitedMarker', LastZone)
		end
	end
end)
