...
RegisterCommand("fazerrefem",function(source, args)
	...
	if not canTakeHostage then 
		drawNativeNotification("Precisas de ter uma pistola para fazer um refem!")
	end
	...
	if not holdingHostageInProgress and canTakeHostage then		
		...
		if closestPlayer ~= nil then
			...
			holdingHostage = true 
			TriggerServerEvent('erp_customscripts:sync', closestPlayer, lib,lib2, anim1, anim2, distans, distans2, height,target,length,spin,controlFlagMe,controlFlagTarget,animFlagTarget,attachFlag)
		else
			drawNativeNotification("Ninguem ao pé de ti para fazeres um refem!")
		end 
	end
	canTakeHostage = false 
end,false)

RegisterNetEvent('erp_customscripts:syncTarget')
AddEventHandler('erp_customscripts:syncTarget', function(target, animationLib, animation2, distans, distans2, height, length,spin,controlFlag,animFlagTarget,attach)
...
RegisterNetEvent('erp_customscripts:syncMe')
AddEventHandler('erp_customscripts:syncMe', function(animationLib, animation,length,controlFlag,animFlag)
	local playerPed = GetPlayerPed(-1)
	ClearPedSecondaryTask(GetPlayerPed(-1))
	RequestAnimDict(animationLib)
	while not HasAnimDictLoaded(animationLib) do
...
RegisterNetEvent('erp_customscripts:cl_stop')
AddEventHandler('erp_customscripts:cl_stop', function()
	holdingHostageInProgress = false
	beingHeldHostage = false 
	...
	while true do 
		if holdingHostage then
			if GetEntityHealth(GetPlayerPed(-1)) <= 102 then --You may need to edit this death check for your server
				...
				local closestPlayer = GetClosestPlayer(2)
				target = GetPlayerServerId(closestPlayer)
				TriggerServerEvent("erp_customscripts:stop",target)
				Wait(100)
				releaseHostage()
			end 
			...
			DrawText3D(playerCoords.x,playerCoords.y,playerCoords.z,"[G] para soltar | [H] para matar")
