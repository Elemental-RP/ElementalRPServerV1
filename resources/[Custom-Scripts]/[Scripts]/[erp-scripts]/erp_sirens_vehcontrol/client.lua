			...
			if IsPedInAnyVehicle(playerped, false) then	
				...
				if GetPedInVehicleSeat(veh, -1) == playerped then
					...
					if GetVehicleClass(veh) == 18 then
						...
						DisableControlAction(0, 36, true) -- INPUT_CHARACTER_WHEEL 
						...
						if not IsPauseMenuActive() then
							...
							-- TOG DFLT SRN LIGHTS
							if IsDisabledControlJustReleased(0, 85) or IsDisabledControlJustReleased(0, 85) then
							...
							-- TOG LX SIREN
							elseif IsDisabledControlJustReleased(0, 36) or IsDisabledControlJustReleased(0, 82) then
