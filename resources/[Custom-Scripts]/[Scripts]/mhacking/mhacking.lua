	...
	while true do
		...
		if showHelp then
			if helpTimer > GetGameTimer() then
				showHelpText("Navegue com ~y~W, A, S, D~s~ e confirma com ~y~SPACE~s~ para o código à esquerda.")
			elseif helpTimer > GetGameTimer()-helpCycle then
				showHelpText("Usa ~y~Teclas de seta ~s~ e ~y~ENTER~s~ para o código à direita")
			else
