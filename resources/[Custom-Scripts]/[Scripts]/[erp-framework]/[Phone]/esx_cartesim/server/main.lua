		...
		MySQL.Async.fetchAll(
			...
			function(result)
				...
				MySQL.Async.execute(
					...
					function (rowsChanged)
					TriggerClientEvent('esx:showNotification', source, "Tens um novo cartão SIM")
					end
				)
		end)
end
...
AddEventHandler('esx_cartesim:sim_give', function (store, index)
  ...
  TriggerClientEvent('esx:showNotification', _source, "Deste o cartão SIM ~o~" .. simZ)
  TriggerClientEvent('esx:showNotification', id, "Compraste um cartão SIM~o~" .. simZ)
