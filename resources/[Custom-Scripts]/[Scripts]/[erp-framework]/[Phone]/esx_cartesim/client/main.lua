...
function OpenSimMenu()
	...
	local elements2 = {}
	  ...
	  ESX.TriggerServerCallback('esx_cartesim:GetList', function(sim)
		  ...
		  {
			  title    = 'Lista de Cartões Sim',
			  elements = elements,
		  },
	  function(data, menu)
  
		local elements2 = {
			{label = 'Usar', value = 'sim_use'},
			{label = 'Dar', value = 'sim_give'},
		  	{label = 'Apagar', value = 'sim_delete'}
		  }
  
		  ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'sim_change', {
			title    = "O que queres fazer com o Sim Card?",
			elements = elements2,
		  ...
		  function(data2, menu2)
			...
			if data2.current.value == 'sim_use' then
				...
				ESX.ShowNotification("Ativaste o cartão SIM ~o~" .. data.current.value.number)
			...
			if data2.current.value == 'sim_delete' then
				...
				ESX.ShowNotification("Usaste o cartão SIM ~o~" .. data.current.value.number)
				TriggerServerEvent('gcPhone:allUpdate')
			end
		...
		if data.current.value == 'open_phone' then
			...
			ESX.TriggerServerCallback('esx_cartesim:CheckSim', function(sim)
				if sim then
					...
					ESX.ShowNotification("Não tens um cartão SIM!")
				end
			end)
