...
AddEventHandler('gcPhone:buyMarket', function(itemName, amount, price)
	...
	if xPlayer.getBank() >= price then
		...
		if sourceItem.limit ~= -1 and (sourceItem.count + amount) > sourceItem.limit then
			TriggerClientEvent('esx:showNotification', _source, "tu não ~r~tens~s~ ~y~espaço livre~s~ suficiente no teu inventário!")
		else
			...
			TriggerClientEvent('esx:showNotification', _source, "" .. price .. " recibo " .. itemName .. ".")
		end
	else
		local missingMoney = price - xPlayer.getBank()
		TriggerClientEvent('esx:showNotification', _source, "tu não tens dinheiro ~r~suficiente~s~, ~y~falta-te~s~ ~r~" .. missingMoney .. "€~s~!")
	end
end)
