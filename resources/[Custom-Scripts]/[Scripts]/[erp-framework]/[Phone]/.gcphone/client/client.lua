...
function ShowNoPhoneWarning () 
  ...
  ESX.ShowNotification("Não tens um ~r~telefone~s~")
end

RegisterNetEvent('general:openphone')
AddEventHandler('general:openphone', function()
...
AddEventHandler("gcPhone:receiveMessage", function(message)
  ...
  if message.owner == 0 then
    local text = '~o~Nova Menssagem'
    if ShowNumberNotification == true then
      text = '~o~Nova Menssagem de ~y~'.. message.transmitter
      for _,contact in pairs(contacts) do
        if contact.number == message.transmitter then
          text = '~o~Nova Menssagem de ~g~'.. contact.display
          break
        end
      end
    end
