...
RegisterServerEvent("erp-inventoryhud_trunk:getOwnedVehicule")
AddEventHandler(
  "erp-inventoryhud_trunk:getOwnedVehicule",
  function()
    local vehicules = {}
    ...
    MySQL.Async.fetchAll(
      ...
      function(result)
        ...
        TriggerClientEvent("erp-inventoryhud_trunk:setOwnedVehicule", _source, vehicules)
      end
    )
  end
)
...
AddEventHandler("esx_trunk:getItem", function(plate, type, item, count, max, owned)
    ...
    if type == "item_standard" then
      local targetItem = xPlayer.getInventoryItem(item)
        if xPlayer.canCarryItem(item, targetItem.count) then
        TriggerEvent(
          "esx_trunk:getSharedDataStore",
          plate,
          function(store)
            ...
            TriggerClientEvent("erp-inventoryhud:refreshTrunkInventory", _source, data, blackMoney, items, weapons)
          end
        )
      else
        TriggerClientEvent(
