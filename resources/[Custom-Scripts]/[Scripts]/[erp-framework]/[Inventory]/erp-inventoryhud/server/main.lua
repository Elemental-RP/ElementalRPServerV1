...
RegisterServerEvent("erp-inventoryhud:tradePlayerItem")
AddEventHandler(
	"erp-inventoryhud:tradePlayerItem",
	function(from, target, type, itemName, itemCount)
		...
		if type == "item_standard" then
			...
			if itemCount > 0 and sourceItem.count >= itemCount then
				if sourceXPlayer.canCarryItem(itemName, targetItem.count) then
				else
...
RegisterCommand(
	...
	function(source, args, rawCommand)
		if IsPlayerAceAllowed(source, "inventory.openinventory") then
			...
			if targetXPlayer ~= nil then
				TriggerClientEvent("erp-inventoryhud:openPlayerInventory", source, target, targetXPlayer.name)
			else
