        ...
        if distance <= 25.0 and hasjob then
            ...
            if distance <= 1.5 then
                ...
                if IsControlJustReleased(0, 38) then
                    TriggerServerEvent('erp-trucking:jobMenu')
                end
            end
        end
        Wait(sleep)
...
RegisterNetEvent('erp-trucking:start')
AddEventHandler('erp-trucking:start', function(data, jobid)
    working = true
    ...
    DeleteVehicle(truck)
    working = false
    TriggerServerEvent('erp-trucking:jobFinished', jobid, damages)
end)

RegisterNetEvent('erp-trucking:menu')
AddEventHandler('erp-trucking:menu', function(data)
    ...
    ESX.UI.Menu.Open(
        ...
        function(data, menu)
            TriggerServerEvent('erp-trucking:startJob', data.current.value)
            menu.close()
        end,
    function(data, menu)
        menu.close()
