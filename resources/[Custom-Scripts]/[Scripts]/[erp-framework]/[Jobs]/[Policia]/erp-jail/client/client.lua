...
AddEventHandler("esx:playerLoaded", function(newData)
	...
	ESX.TriggerServerCallback("erp-jail:retrieveJailTime", function(inJail, newJailTime)
		if inJail then
...
RegisterNetEvent("erp-jail:openJailMenu")
AddEventHandler("erp-jail:openJailMenu", function()
	OpenJailMenu()
end)

RegisterNetEvent("erp-jail:jailPlayer")
AddEventHandler("erp-jail:jailPlayer", function(newJailTime)
	jailTime = newJailTime
...
RegisterNetEvent("erp-jail:unJailPlayer")
AddEventHandler("erp-jail:unJailPlayer", function()
	jailTime = 0
...
function JailLogin()
	...
	ESX.ShowNotification("Na última vez em que dormiste, foste preso, por causa disso agora foste reacolocado!")
...
function UnJail()
	InJail()
	tp = vector3(1845.6022949219,2585.8029785156,45.672061920166)
	ESX.Game.Teleport(PlayerPedId(), tp)

	ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin)
		TriggerEvent('skinchanger:loadSkin', skin)
	end)

	ESX.ShowNotification("Estás libertado, fica calmo lá fora! Boa sorte!")
end
...
function InJail()
	...
	Citizen.CreateThread(function()
		...
		while jailTime > 0 do
			...
			ESX.ShowNotification("Foste preso por " .. jailTime .. " anos!")

			TriggerServerEvent("erp-jail:updateJailTime", jailTime)

			if jailTime == 0 then
				UnJail()

				TriggerServerEvent("erp-jail:updateJailTime", 0)
			end
					...
					if DistanceCheck <= 1.5 then
						...
						if IsControlJustPressed(0, 38) then
							...
							if v["state"] then
								...
								ESX.ShowNotification("Já pegaste este pacote!")
							end
...
function PackPackage(packageId)
	...
	while Packaging do
		...
		if PackPercent >= 100 then
			...
			ESX.Game.Utils.DrawText3D(Package, "A Embalar... " .. math.ceil(tonumber(PackPercent)) .. "%", 0.4)
		end
...
function DeliverPackage(packageId)
	...
	while Packaging do
		...
		if not IsEntityAttachedToEntity(packageId, PlayerPedId()) then
			...
			ESX.Game.Utils.DrawText3D(DeliverPosition, "[E] Largar Pacote", 0.4)
			...
			if DistanceCheck <= 2.0 then
				if IsControlJustPressed(0, 38) then
					...
					TriggerServerEvent("erp-jail:prisonWorkReward")
				end
			end
		end
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'jail_prison_menu',
		{
			...
			elements = {
				...
				{ label = "Libertar Cidadão", value = "unjail_player" }
			}
		},
	function(data, menu)
						...
						function(data3, menu3)
						  	...
						  	if reason == nil then
								...
								if closestPlayer == -1 or closestDistance > 3.0 then
								  	...
								  	TriggerServerEvent("erp-jail:jailPlayer", GetPlayerServerId(closestPlayer), jailTime, reason)
								end
		...
		elseif action == "unjail_player" then
			...
			ESX.TriggerServerCallback("erp-jail:retrieveJailedPlayers", function(playerArray)

				if #playerArray == 0 then
					ESX.ShowNotification("A Tua prisão está vazia!")
					return
				end

				for i = 1, #playerArray, 1 do
					table.insert(elements, {label = "Prisioneiro: " .. playerArray[i].name .. " | Tempo de Prisão: " .. playerArray[i].jailTime .. " anos", value = playerArray[i].identifier })
				end

				ESX.UI.Menu.Open(
					'default', GetCurrentResourceName(), 'jail_unjail_menu',
					{
						title = "Libertar Cidadão",
						align = "center",
						elements = elements
					},
				function(data2, menu2)
					...
					TriggerServerEvent("erp-jail:unJailPlayer", action)
