RegisterCommand("menudeprisão", function(source, args)

	if PlayerData.job.name == "police" then
		OpenJailMenu()
	else
		ESX.ShowNotification("Não és um oficial!")
	end
end)
...
function TeleportPlayer(pos)
	...
	if #Values["goal"] > 1 then
		...
		function(data, menu)
			...
			if action == "Boiling Broke" or action == "Segurança" then
				if PlayerData.job.name ~= "police" then
					ESX.ShowNotification("Não tens uma chave para ir aqui!")
					return
				end
			end
...
Citizen.CreateThread(function()
    ...
    AddTextComponentString('Penitenciária Boilingbroke')
