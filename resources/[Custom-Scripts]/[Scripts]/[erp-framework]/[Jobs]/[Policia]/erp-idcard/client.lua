...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(10)
		if (IsControlJustPressed(2, 56)) and GetLastInputMethod(0)	then
				ESX.UI.Menu.Open(
					'default', GetCurrentResourceName(), 'id_card_menu',
					{
						...
						align    = 'bottom-right',
						elements = {
							{label = 'Ver o meu próprio CC', value = 'check'},
							{label = 'Mostrar CC', value = 'show'}
						}
					},
					function(data2, menu2)
						if data2.current.value == 'check' then
							TriggerServerEvent('erp-idcard:open', GetPlayerServerId(PlayerId()), GetPlayerServerId(PlayerId()))
							menu2.close()
						elseif data2.current.value == 'show' then
							local player, distance = ESX.Game.GetClosestPlayer()

							if distance ~= -1 and distance <= 3.0 then
								TriggerServerEvent('erp-idcard:open', GetPlayerServerId(PlayerId()), GetPlayerServerId(player))
								menu2.close()
							else
								ESX.ShowNotification('Não está ninguem ao pé de ti')
								menu2.close()
							end
						end
					end,
...
RegisterNetEvent('erp-idcard:open')
AddEventHandler('erp-idcard:open', function(playerData)
	cardOpen = true
	SendNUIMessage({
		action = "open",
		array = playerData
