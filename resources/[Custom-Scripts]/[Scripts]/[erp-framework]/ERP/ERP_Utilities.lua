...
function ERPUtils:DrawTextTemplate(text,x,y,font,scale1,scale2,colour1,colour2,colour3,colour4,wrap1,wrap2,centre,outline,dropshadow1,dropshadow2,dropshadow3,dropshadow4,dropshadow5,edge1,edge2,edge3,edge4,edge5)
    return {
      text         =                    "",
      x            =                    -1,
      y            =                    -1,
...
function ERPUtils.PointOnSphere(alt,azu,radius,orgX,orgY,orgZ)
...
function ERPUtils.ClampCircle(x,y,radius)
...
function ERPUtils.GetHashKey(strToHash)
...
GetHashKey     = ERPUtils.GetHashKey

function ERPUtils:FindNearestZone(position, table)
    ...
    if zone.Positions then 
      for act,pos in pairs(zone.Positions) do
        if type(pos) == 'vector3' then
          local curDist = ERPUtils:GetVecDist(position, pos)
...
function ERPUtils:GetVecDist(v1,v2)
  if not v1 or not v2 or not v1.x or not v2.x then return 0; end
  return math.sqrt(  ( (v1.x or 0) - (v2.x or 0) )*(  (v1.x or 0) - (v2.x or 0) )+( (v1.y or 0) - (v2.y or 0) )*( (v1.y or 0) - (v2.y or 0) )+( (v1.z or 0) - (v2.z or 0) )*( (v1.z or 0) - (v2.z or 0) )  )
end

function ERPUtils.GetCoordsInFrontOfCam(...)
...
function ERPUtils.RotationToDirection(rot)
...
function ERPUtils.LoadModel(model, wait)
  local hk = ERPUtils.GetHashKey(model)
  if wait then
...
function ERPUtils.ReleaseModel(model)
  local hk = ERPUtils.GetHashKey(model)
...
function ERPUtils:LoadModelTable(table)
  if type(table) ~= 'table' then return false; end
  for k,v in pairs(table) do
    if type(v) == 'string' then
      local hk = ERPUtils.GetHashKey(v)
      while not HasModelLoaded(hk) do
        RequestModel(hk)
        Citizen.Wait(0)
      end
...
function ERPUtils:ReleaseModelTable(table)
  if type(table) ~= 'table' then return false; end
  for k,v in pairs(table) do
    if type(v) == 'string' then
      local hk = ERPUtils.GetHashKey(v)
...
function ERPUtils.LoadWeaponTable(table)  
  if type(table) ~= 'table' then return false; end
  for k,v in pairs(table) do
    if type(v) == 'string' then
      local hk = ERPUtils.GetHashKey(v)
...
function ERPUtils.ReleaseWeaponTable(table)
  if type(table) ~= 'table' then return false; end
  for k,v in pairs(table) do
    if type(v) == 'string' then
      local hk = ERPUtils.GetHashKey(v)
...
function ERPUtils.LoadAnimTable(table)
  if type(table) ~= 'table' then return false; end
  for k,v in pairs(table) do
    if type(v) == 'string' then
...
function ERPUtils.ReleaseAnimTable(table)
  if type(table) ~= 'table' then return false; end
  for k,v in pairs(table) do
    if type(v) == 'string' then
...
function ERPUtils:LoadAnimDict(dict)
...
function ERPUtils:ReleaseAnimDict(dict)
  if type(dict) ~= 'string' then return false; end
...
function ERPUtils.NetworkControlEntity(ent)
...
function ERPUtils.NetworkControlDoor(obj)
