...
document.onkeyup = function (data) {
    if (data.which == 27) { // Escape key
        $.post('http://erp-notepad/escape', JSON.stringify({}));
        if (notepadreader == true) {
            var x = document.getElementById("p1").value;
            $.post('http://erp-notepad/updating', JSON.stringify({ text: x}));
            $("#main").fadeOut();
...
function dropNotepad() {
    var x = document.getElementById("p1").value;
    if (x == ""){
        $.post('http://erp-notepad/droppingEmpty', JSON.stringify({}));
    }else {
        $.post('http://erp-notepad/dropping', JSON.stringify({ text: x}));
        $("#main").fadeOut();
        document.getElementById("p1").value = "";
        $("#main").css('display', 'none');
    }   
