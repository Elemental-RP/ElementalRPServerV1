...
function OpenShopMenu()
	...
	function(data, menu)
		if data.current.value == 'outfit' then
			...
			ESX.TriggerServerCallback('loaf_housing:getPlayerDressing', function(dressing)
						...
						if data.current.value == 'change_cloth' then
							...
							ESX.TriggerServerCallback('loaf_housing:getPlayerDressing', function(dressing)
								...
								function(data2, menu)
									...
									ESX.TriggerServerCallback('erp-skinshop:buyClothes', function(bought)
										if bought then
			...
			TriggerEvent("erp-skinshop:toggleMenu", "clotheshop")
		end
		currentAction     = 'shop_menu'
		...
		if data.current.value == 'yes' then
			ESX.TriggerServerCallback('erp-skinshop:buyClothes', function(bought)
				if bought then
					...
					ESX.TriggerServerCallback('erp-skinshop:checkPropertyDataStore', function(foundStore)
						if foundStore then
							ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'save_dressing', {
								...
								if data2.current.value == 'yes' then
									ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'outfit_name', {
										...
										TriggerEvent('skinchanger:getSkin', function(skin)
											TriggerServerEvent('erp-skinshop:saveOutfit', data3.value, skin)
...
RegisterNUICallback("endDialog", function()
	TriggerEvent("erp-skinshop:toggleMenu")
	openDialog()
end)

AddEventHandler('erp-skinshop:hasEnteredMarker', function(zone)
	currentAction     = 'shop_menu'
	currentActionMsg  = _U('press_menu')
	currentActionData = {}
end)

AddEventHandler('erp-skinshop:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	currentAction = nil
	inMenu = false
...
Citizen.CreateThread(function()
	while true do
		...
		if (isInMarker and not hasAlreadyEnteredMarker) or (isInMarker and lastZone ~= currentZone) then
			...
			TriggerEvent('erp-skinshop:hasEnteredMarker', currentZone)
		end

		if not isInMarker and hasAlreadyEnteredMarker then
			hasAlreadyEnteredMarker = false
			TriggerEvent('erp-skinshop:hasExitedMarker', lastZone)
		end
