$(function(){
    ...
    document.onkeydown = function(data){ // document.onkeyup
        if(data.which == 27){
            $.post('http://erp-skinshop/exit');
        }
        if(data.which == 65 || (data.which == 65 && data.repeat)){ //a if
            //lastTime = Date.now();
            $.post('http://erp-skinshop/rotate', JSON.stringify("left"));
        }
        if(data.which == 68 || (data.which == 68 && data.repeat)) { //d if
            //lastTime = Date.now();
            $.post('http://erp-skinshop/rotate', JSON.stringify("right"));
        }
        if(data.which == 39) { // right
            //alert("Teste")
            $.post('http://erp-skinshop/color', JSON.stringify("right"));
        }
        if(data.which == 37) { // left
            //alert("Teste")
            $.post('http://erp-skinshop/color', JSON.stringify("left"));
        }
    }
...
function menu(){
    $(".classes").each(function(i, ojb){
        ...
        $(this).click(function(){       
            if (menu == "mascara" || menu == "chapeu" || menu == "oculos" || menu == "orelha"){
                $.post('http://erp-skinshop/zoom', JSON.stringify("cara"));
            }
            else if (menu == "sapato"){
                $.post('http://erp-skinshop/zoom', JSON.stringify("zapatos"));
            }
            else if (menu == "gravata" || menu == "blusa" || menu == "jaqueta" || menu == "colete"){
                $.post('http://erp-skinshop/zoom', JSON.stringify("tops"));
            }
            else if (menu == "calca"){
                $.post('http://erp-skinshop/zoom', JSON.stringify("pants"));
            }
            else{
                $.post('http://erp-skinshop/zoom', JSON.stringify("ropa"));
            }
    ...
    $("#confirm").each(function(i, ojb){
        $(this).click(function(){       
            $.post('http://erp-skinshop/endDialog');
        });
    });
    ...
    $(".botao").each(function(i, ojb){
        $(this).click(function(){
            ...
            $.post("http://erp-skinshop/update", JSON.stringify([dados, tipo]));
        })
    })
}