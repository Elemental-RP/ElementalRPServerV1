...
RegisterServerEvent('erp-robbery:tooFar')
AddEventHandler('erp-robbery:tooFar', function(currentStore)
	...
	for i=1, #xPlayers, 1 do
		...
		if xPlayer.job.name == 'police' then
			...
			TriggerClientEvent('erp-robbery:killBlip', xPlayers[i])
		end
	end

	if robbers[_source] then
		TriggerClientEvent('erp-robbery:tooFar', _source)
		robbers[_source] = nil
		TriggerClientEvent('esx:showNotification', _source, _U('robbery_cancelled_at', Stores[currentStore].nameOfStore))
	end
end)

RegisterServerEvent('erp-robbery:robberyStarted')
AddEventHandler('erp-robbery:robberyStarted', function(currentStore)
	local _source  = source
			...
			if cops >= Config.PoliceNumberRequired then
				...
				for i=1, #xPlayers, 1 do
					...
					if xPlayer.job.name == 'police' then
						...
						TriggerClientEvent('erp-robbery:setBlip', xPlayers[i], Stores[currentStore].position)
					end
				end
				...
				TriggerClientEvent('erp-robbery:currentlyRobbing', _source, currentStore)
				TriggerClientEvent('erp-robbery:startTimer', _source)
				...
				SetTimeout(store.secondsRemaining * 1000, function()
					if robbers[_source] then
						rob = false
						if xPlayer then
							TriggerClientEvent('erp-robbery:robberyComplete', _source, store.reward)
