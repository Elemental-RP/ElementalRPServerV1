...
RegisterNetEvent('erp-robbery:currentlyRobbing')
AddEventHandler('erp-robbery:currentlyRobbing', function(currentStore)
	holdingUp, store = true, currentStore
end)

RegisterNetEvent('erp-robbery:killBlip')
AddEventHandler('erp-robbery:killBlip', function()
	RemoveBlip(blipRobbery)
end)

RegisterNetEvent('erp-robbery:setBlip')
AddEventHandler('erp-robbery:setBlip', function(position)
...
RegisterNetEvent('erp-robbery:tooFar')
AddEventHandler('erp-robbery:tooFar', function()
	holdingUp, store = false, ''
	ESX.ShowNotification(_U('robbery_cancelled'))
end)

RegisterNetEvent('erp-robbery:robberyComplete')
AddEventHandler('erp-robbery:robberyComplete', function(award)
	holdingUp, store = false, ''
	ESX.ShowNotification(_U('robbery_complete', award))
end)

RegisterNetEvent('erp-robbery:startTimer')
AddEventHandler('erp-robbery:startTimer', function()
	...
	while true do
					...
					if distance < 0.5 then
						...
						if IsControlJustReleased(0, 38) then
							if IsPedArmed(PlayerPedId(), 4) then
								TriggerServerEvent('erp-robbery:robberyStarted', k)
							else
		...
		if holdingUp then
			...
			if Vdist(playerPos.x, playerPos.y, playerPos.z, storePos.x, storePos.y, storePos.z) > Config.MaxDistance then
				TriggerServerEvent('erp-robbery:tooFar', store)
			end
		end
	end
end)
