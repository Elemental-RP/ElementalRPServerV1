...
function setUniform(uniform, playerPed)
	TriggerEvent('skinchanger:getSkin', function(skin)
		...
		if uniformObject then
			...
			if uniform == 'colete_wear' or uniform == 'fepricoleteoficial' then
...
function OpenCloakroomMenu()
	...
	local elements = {
		...
		{label = _U('coleter2_wear'), uniform = 'coleter2_wear'},
		...
		--{label = _U('police_wear'), uniform = grade}, //each job can have only one uniform
		-- custom
	}

	-- each job can have multiple unirmorfs only for them
	...
	elseif grade == 'subint' then
		...
		table.insert(elements, {label = 'Farda Corta Vento c/ Colete', uniform = 'fcomissariocortaventocolete'})
		table.insert(elements, {label = '---------TRÂNSITO---------', uniform = 'ttt_wear'})
		table.insert(elements, {label = 'PSP Trânsito Colete Fechado', uniform = 'ftransitocoleteorioficial'})
		table.insert(elements, {label = 'PSP Trânsito Colete Aberto Amarelo', uniform = 'ftransitocoletaberto1'})
		table.insert(elements, {label = 'PSP Trânsito Colete Aberto Laranja', uniform = 'ftransitocoletaberto2'})
...
function OpenPoliceActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'police_actions', {
		...
		elements = {
			...
			{label = _U('roupa_viatura'), 		value = 'roupa_viatura'}
	}}, function(data, menu)
		if data.current.value == 'jail_menu' then
            TriggerEvent("erp-jail:openJailMenu")
		end
	...
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'lookup_vehicle', {
		...
		if data.value == nil or length < 2 or length > 13 then
			...
			ESX.TriggerServerCallback('esx_policejob:getVehicleFromPlate', function(owner, found)
				if found then
					exports['mythic_notify']:SendAlert('success', _U('search_database_found', owner))
				else
					exports['mythic_notify']:SendAlert('error', _U('search_database_error_not_found'))
				end
			end, data.value)
			menu.close()
		end
	...
	while true do
		...
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'police' then
			...
			for k,v in pairs(Config.PoliceStations) do
				...
				if Config.EnablePlayerManagement and ESX.PlayerData.job.grade_name == 'diretornacional' or ESX.PlayerData.job.grade_name == 'comissario' or ESX.PlayerData.job.grade_name == 'subc' then
