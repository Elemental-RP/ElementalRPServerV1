...
--jobs garage stuff---------------------------------
RegisterServerEvent('esx_ambulancejob:setJobVehicleState')
AddEventHandler('esx_policejob:setJobVehicleState', function(plate, state)
	...
	MySQL.Async.execute('UPDATE owned_vehicles_jobs SET `stored` = @stored WHERE plate = @plate AND job = @job', {
		...
		if rowsChanged == 0 then
			print(('esx_vehicleshop_ambulance: %s exploited the garage!'):format(xPlayer.identifier))
		end
	end)
end)
