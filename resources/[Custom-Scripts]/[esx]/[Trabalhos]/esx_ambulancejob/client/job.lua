        ...
        for _, menuConfig in pairs(menuConfigs) do
            ...
            if menuConfig:enableMenu() then
				        ...
				        local keybindControl = menuConfig.data.keybind
								...
								if ((IsControlJustReleased(0, keybindControl)) and (ESX.PlayerData.job.name == 'ambulance')) then
									-- Init UI
									showMenu = true
									SendNUIMessage({
										type = 'init',
...
RegisterCommand("InemSubMenu1", function(source, args, rawCommand)
    ...
    SendNUIMessage({
        type = 'init',
        data = subMenuConfigs["InemSubMenu1"].data,
        resourceName = GetCurrentResourceName()
    })
    SetNuiFocus(true, true)
end, false)
...
RegisterCommand("reanimar", function(source, args, raw)
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'ambulance' then
		local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
		...
		exports['mythic_notify']:SendAlert('error', 'Não és do Inem')
	end
end, false)

RegisterCommand("curarferidaspequenas", function(source, args, raw)
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'ambulance' then
		local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
		ESX.TriggerServerCallback('esx_ambulancejob:getItemAmount', function(quantity)
			if quantity > 0 then
...
RegisterCommand("tratarferidasgraves", function(source, args, raw)
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'ambulance' then
		local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
		ESX.TriggerServerCallback('esx_ambulancejob:getItemAmount', function(quantity)
			if quantity > 0 then
...
RegisterCommand("colocarnoveiculo", function(source, args, raw)
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'ambulance' then
		local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
		TriggerServerEvent('esx_ambulancejob:putInVehicle', GetPlayerServerId(closestPlayer))
	else
...
RegisterCommand("procurarcidadao", function(source, args, raw)
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'ambulance' then
		TriggerServerEvent('esx_ambulancejob:svsearch')
	else
		exports['mythic_notify']:SendAlert('error', 'Não és do Inem')
