...
RegisterNUICallback('CancelRegistro', function(data, cb)
	if hasIdentity then
		EnableGui(false)
	else
		TriggerEvent('chat:addMessage', { args = { '^1[REGISTRO]', '^1  Deves criar a tua primeira personagem para poder jogar' } })
	end
end)
		...
		elseif theData == "height" then
			...
			if height then
				if height > 200 or height < 140 then
					reason = "Altura do jogador inaceitável!"
					break
				end
			else
				reason = "Altura do jogador inaceitável!"
...
function verifyName(name)
	...
	if nameLength > 25 or nameLength < 2 then
		return 'O nome do teu jogador é muito curto ou muito longo.'
	end
	...
	if count ~= nameLength then
		return 'O teu nome de jogador contém caracteres especiais que não são permitidos.'
	end
	...
	if spacesInName > 2 then
		return 'O teu nome contém mais de dois espaços'
	end
